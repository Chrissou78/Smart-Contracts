...
contract GremsNFT is ERC721A, SharedOwnable, ReentrancyGuard {
    using Strings for uint256;
    ...
    function mint(uint256 _mintAmount) public payable mintCompliance(_mintAmount) mintPriceCompliance(_mintAmount) {
        ...
        if (cost != 0) {
            require(msg.value >= cost * _mintAmount, "Not Enough BNB");
            require(payable(Treasury).send(msg.value));
        } 
        if(whitelistMintEnabled) require(isWhitelisted[msg.sender], "Must be Whitelisted to Mint");
        _safeMint(_msgSender(), _mintAmount);
        NFTMinted += _mintAmount;
        if (NFTMinted >= Draw*maxTokenIdPerPhase) {
            DrawWinner(Draw);
            Winners[Index] = winner1;
            Index+=1;
            Winners[Index] = winner2;
            Index+=1;
            Winners[Index] = winner3;
            Index+=1;
            Draw +=1;   
        }
        if(NFTMinted == maxSupply) {
            DrawWinner(Draw);
            Winners[Index] = winner1;
            Index+=1;
            Winners[Index] = winner2;
    ...
    //******************************************************************************************************
    function Initialise(uint256 _cost, uint256 _maxSupply, uint256 _maxMintAmountPerTx, uint256 _maxMintAmountPerWallet, uint256 _tokenId, address treasury) public onlySharedOwners {
        cost = _cost;
    ...
    function setHiddenMetadataUri(string memory _hiddenMetadataUri) public onlySharedOwners {hiddenMetadataUri = _hiddenMetadataUri;}
    ...
    function setRevealed(bool _state) public onlySharedOwners {revealed = _state;}
    ...
    function withdrawBNB() public onlySharedOwners nonReentrant {
        (bool os, ) = payable(owner()).call{value: address(this).balance}("");
        require(os);
    }

    function DrawWinner(uint draw) onlySharedOwners public {
        require (draw > 0 && draw <= 11, "maw 11 draws");
        require (NFTMinted > draw*maxTokenIdPerPhase, "need to mint the according number of NFT to Draw Lottery");


        if (draw <= 10) {
            winner1 = (random() % maxTokenIdPerPhase) + (draw-1)*maxTokenIdPerPhase;
            counter += 3331*draw;
            winner2 = (random() % maxTokenIdPerPhase) + (draw-1)*maxTokenIdPerPhase;
            counter += 1171*draw;
            winner3 = (random() % maxTokenIdPerPhase) + (draw-1)*maxTokenIdPerPhase;
        }
        else if (draw == 11){
            winner1 = random() % maxSupply;
            counter += 6053*draw;
            winner2 = random() % maxSupply;
            counter += 4723*draw;
            winner3 = random() % maxSupply;
        }
    }
    //******************************************************************************************************
    // Internal functions

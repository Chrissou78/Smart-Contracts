...
contract GremsNFT is ERC721A, SharedOwnable, ReentrancyGuard {
    ...
    function DrawWinner(uint Draw) onlySharedOwners public returns (uint256 W1, uint256 W2, uint256 W3){
        require (Draw > 0 && Draw <= 11, "maw 11 draws");
        require (NFTMinted > Draw*1000, "need to mint the according number of NFT to Draw Lottery");
        uint256 winner1;
        uint256 winner2;
        uint256 winner3;

        if (Draw <= 10) {
            winner1 = (random() % 1000) + (Draw-1)*1000;
            counter += 3331*Draw;
            winner2 = (random() % 1000) + (Draw-1)*1000;
            counter += 1171*Draw;
            winner3 = (random() % 1000) + (Draw-1)*1000;
        }
        else if (Draw == 11){
            winner1 = random() % 10000;
            counter += 6053*Draw;
            winner2 = random() % 10000;
            counter += 4723*Draw;
            winner3 = random() % 10000;
        }

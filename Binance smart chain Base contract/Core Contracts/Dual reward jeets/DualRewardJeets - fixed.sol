...
abstract contract SharedOwnable is Ownable {
    ...
    modifier onlySharedOwners() {require(_sharedOwners[msg.sender], "not shared owner"); _;}
...
contract Main is BEP20, SharedOwnable, SafeToken {
    ...
    address private Token2Adress;
    address [] List; 
        
    bool private swapping;
    bool private swapAndLiquifyEnabled = true;
    bool private BurnReward1Option = false;
    bool private BurnReward2Option = false;
    ...
    DIVIDENDTracker2 private dividendTracker2;
    ...
    uint256 private InjectedGains1 = 0;
    uint256 private InjectedGains2 = 0;
    ...
    uint256 private _MultiUseFee= 1;
    uint256 private _StakingFee= 0;
    uint256 private _token1DividendRewardsFee = 0;
    uint256 private _token2DividendRewardsFee = 0;
    uint256 private _Wallet2WalletFee = 0; // no wallet to wallet fee
    uint256 private _BuyFee = 5;
    uint256 private _SellFee = 0;
    uint256 private VminDiv = 1;
    uint256 private VmaxDiv = 10;
    uint256 private MaxJeetsFee = 15;
    ...
    constructor(string memory name_, string memory symbol_, uint256 decimal_, address reward1_, address reward2_, address marketing_, address team_, address multiuse_, uint256 supply_, uint256 dispatch_) BEP20(name_, symbol_) {
        ...
        TOKEN2 = IBEP20(Token2Adress);
        dividendTracker1 = new DIVIDENDTracker1("Rwd1_T", "RDT1", Token1Adress, minTokenBalanceForDividends, decimal, Creator);
    	dividendTracker2 = new DIVIDENDTracker2("Rwd2_T", "RDT2", Token2Adress, minTokenBalanceForDividends, decimal, Creator);

	    IUniswapV2Router02 _uniswapV2Router = IUniswapV2Router02(0x10ED43C718714eb63d5aA57B78B54704E256024E); // Prod : 0x10ED43C718714eb63d5aA57B78B54704E256024E || Test : 0x9Ac64Cc6e4415144C455BD8E4837Fea55603e5c3
    ...
    // Public functions
    //******************************************************************************************************
    function decimals() public view returns (uint256) { return decimal; }
    ...
    function GetFees() public view returns(uint Buy, uint Sell, uint Wallet2Wallet, uint Liquidity, uint Marketing, uint Team, uint MultiUse, uint Staking, uint Dividend1, uint Dividend2, uint Burn){return (_BuyFee, _SellFee, _Wallet2WalletFee, _LiquidityFee, _MarketingFee, _TeamFee, _MultiUseFee, _StakingFee, _token1DividendRewardsFee, _token2DividendRewardsFee, _BurnFee);}
    ...
    function GetContractAddresses() public view returns(address marketing, address team, address multiuse, address staking, address reward1, address reward2, address Dead, address LP){return (address(MarketingWallet), address(TeamWallet), address(MultiUseWallet), address(StakingWallet), address(dividendTracker1), address(dividendTracker2), address(DeadWallet), address(uniswapV2Pair));}
    ...
    function getAccountTOKEN2DividendsInfo(address account) external view returns (address, int256, int256, uint256, uint256, uint256, uint256, uint256) {return dividendTracker2.getAccount(account);}
    function GetDividendInfo() external view returns(uint256 Reward1TokenHolders, uint256 Reward2TokenHolders, uint256 Reward1TotalDivDistributed, uint256 Reward2TotalDivDistributed, bool Reward1Burn, bool Reward2Burn, uint256 claimwait) { 
        return(dividendTracker1.getNumberOfTokenHolders(), dividendTracker2.getNumberOfTokenHolders(), dividendTracker1.totalDividendsDistributed(), dividendTracker2.totalDividendsDistributed(), BurnReward1Option, BurnReward2Option, dividendTracker1.claimWait());
    }

    function GetInjectedGains() external view returns (uint256 injectedgains1, uint256 injectedgains2) {return(InjectedGains1, InjectedGains2);}
    ...
    function GetContractBalance() external view returns (uint256 stakingbnb, uint256 marketingbnb, uint256 teambnb , uint256 multiusebnb) {return(stakingPortion, marketingBNBPortion, teamBNBPortion , multiuseBNBPortion);}
    ...
    function getLiquidityUnlockTime() public view returns (uint256 Days, uint256 Hours) {
    ...
    function ChangeReward (address Reward1, address Reward2) external onlySharedOwners {
        if (address(Reward1) != address(TOKEN1)) {
            dividendTracker1.ChangeDividendToken(Reward1);
            TOKEN1 = IBEP20(Reward1); 
            Reward1Burnt = 0;
        }
        if (address(Reward2) != address(TOKEN2)) {
            dividendTracker2.ChangeDividendToken(Reward2);
            TOKEN2 = IBEP20(Reward2); 
            Reward2Burnt = 0;
        }
    }
    ...
    function AddSharedOwner(address account, bool DividendExcluded) public onlySharedOwners {
        ...
        if(DividendExcluded) {
            ...
            dividendTracker2.excludeFromDividends(address(account), true);
        }
    ...
    function RemoveharedOwner(address account) public onlySharedOwners {
        ...
        dividendTracker2.excludeFromDividends(address(account), false);
    ...
    function setProjectWallet (address payable _newMarketingWallet, address payable _newTeamWallet, address payable _newMultiUseWallet, address payable _newStakingWallet) external onlySharedOwners {
        if (_newMarketingWallet != MarketingWallet) {
            ...
            dividendTracker2.excludeFromDividends(MarketingWallet, false);
            ...
            dividendTracker2.excludeFromDividends(_newMarketingWallet, true);
    ...
    function SetDelay (bool delayoption, uint256 mintime) external onlySharedOwners {
        require(mintime <= 28800, "MinTime<1D" );
        MinTime = mintime;
    ...
    function SetLimits(uint256 _maxWallet, uint256 _maxSell, uint256 _minswap, uint256 _swapmax, uint256 newMinimumBalance, uint256 NewGas, uint256 claimWait, uint256 MaxTax, bool _swapAndLiquifyEnabled, bool autoreward, bool autodistribution) external onlySharedOwners {
        uint256 supply = totalSupply ();
        require(_maxWallet * 10**decimal >= supply / 100 && _maxWallet * 10**decimal <= supply, "MawWallet 1% min");
        require(_maxSell * 10**decimal >= supply / 1000 && _maxSell * 10**decimal <= supply, "MawSell 0.1% min" );
        require(_minswap * 10**decimal >= supply / 10000 && _minswap <= _swapmax / 2, "MinSwap 0.01% min" );
        require(newMinimumBalance * 10**decimal <= supply / 100, "MinimumBalance 1% max" );
        require(NewGas >= 300000, "Gas 300000 min");
        require(claimWait >= 3600 && claimWait <= 86400, "claimWait 1/24h");
        require(MaxTax >= 1 && MaxTax <= 25, "Max Tax 1/25%");
        require(_swapmax >= _minswap.mul(2) && _swapmax * 10**decimal <= supply, "MaxSwap SwapMinx2 min" );
    ...
    function SetRewardsOptions(bool Reward1Burn, bool Reward2Burn) external onlySharedOwners() {
        BurnReward1Option = Reward1Burn;
        BurnReward2Option = Reward2Burn;
    } 
  
    function SetTaxes(uint256 newBuyTax, uint256 wallet2walletfee, uint256 newLiquidityTax, uint256 newBurnTax, uint256 newMarketingTax, uint256 newTeamTax, uint256 newMultiUseTax, uint256 newStakingTax, uint256 newReward1Tax, uint256 newReward2Tax) external onlySharedOwners() {
        require(newBuyTax <= MaxTaxes && newBuyTax >= newBurnTax, "Total Tax<MaxTax");
        uint256 TransferTax = newMarketingTax.add(newTeamTax).add(newMultiUseTax).add(newStakingTax);
        require(TransferTax.add(newReward1Tax).add(newReward2Tax).add(newLiquidityTax).add(newBurnTax) <= MaxTaxes, "Total Tax>MaxTax");
        require(newMarketingTax >= 0 && newTeamTax >= 0 && newMultiUseTax >= 0 && newBuyTax >= 0 && newReward1Tax >= 0 && newReward2Tax >= 0 && newLiquidityTax >= 0 && newBurnTax >= 0,"no -");
        if(wallet2walletfee != 0){require(wallet2walletfee >= _BurnFee && wallet2walletfee <= MaxTaxes, "W2W<25%");}
        ...
        _token2DividendRewardsFee = newReward2Tax;
        _SellFee = _LiquidityFee.add(_MarketingFee).add(_TeamFee).add(_MultiUseFee).add(_StakingFee).add(_token1DividendRewardsFee).add(_token2DividendRewardsFee).add(_BurnFee);
    } 
    
    function updateUniswapV2Router(address newAddress) external onlySharedOwners {
        require(newAddress != address(uniswapV2Router), "address=router");
    ...
    function SetExclusions (address account, bool Fee, bool MaxTx, bool Dividend, bool BlackList, bool WhiteList) external onlySharedOwners {
        ...
        dividendTracker2.excludeFromDividends(address(account), Dividend);
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlySharedOwners {
        require(pair != uniswapV2Pair, "MP can't be removed");
	...
	function ExtendLockTime(uint256 newdays, uint256 newhours) external onlySharedOwners {
        ...
        require(lockTimeInSeconds > 0);
        
        //setUnlockTime(lockTimeInSeconds + _liquidityUnlockTime);
        _liquidityUnlockTime = lockTimeInSeconds + _liquidityUnlockTime;
    ...
    /*function ReleaseLP() external onlySharedOwners {
        require(block.timestamp >= _liquidityUnlockTime, "Not unlocked");
    ...
    function SetJeetsTax(bool jeetsfee, bool jeetsburn, bool jeetsstaking, uint256 vmaxdiv, uint256 vmindiv, uint256 maxjeetsfee)  external onlySharedOwners {
        require (vmaxdiv >= 10 && vmaxdiv <= 40, "Vmax 10%/40%");
        require (vmindiv >= 1 && vmindiv <= 10, "Vmin 1%/10%");
        require (maxjeetsfee >= 1 && maxjeetsfee <= 20, "jeets 1/20%");
        JeetsFee = jeetsfee;
        JeetsBurn = jeetsburn;
    ...
    function _setAutomatedMarketMakerPair(address pair, bool value) private onlySharedOwners {
        require(automatedMarketMakerPairs[pair] != value, "already market pair");
        automatedMarketMakerPairs[pair] = value;
 
        if(value) {
            dividendTracker1.excludeFromDividends(pair, true);
            dividendTracker2.excludeFromDividends(pair, true);
        }
    ...
    function _transfer(address from, address to, uint256 amount) internal override {
        ...
        if(amount == 0) {return;}
        if(to != address(this) && to != DeadWallet) require(!_isBlacklisted[from] && !_isBlacklisted[to] , "Blacklisted"); //blacklist function
        // preparation of launch LP and token dispatch allowed even if trading not allowed
        if(!tradingEnabled) {require(_isWhitelisted[from], "no Trading");}
        ...
        if(tradingEnabled && block.number < counter && !_isWhitelisted[to] && automatedMarketMakerPairs[from]) {
            ...
            dividendTracker2.excludeFromDividends(address(to), true);
        }
        ...
        if(!_isWhitelisted[to] && automatedMarketMakerPairs[from]){
            if(to != address(this) && to != DeadWallet){
                ...
                require((heldTokens + amount) <= maxWalletAmount, "amount>maxWallet");
            }
        }
        // Max sell limitation
        if(automatedMarketMakerPairs[to] && (!_isExcludedFromMaxTx[from]) && (!_isExcludedFromMaxTx[to])){require(amount <= maxSellTransactionAmount, "Sell amount> max");}

         if (DelayOption && !_isWhitelisted[from] && automatedMarketMakerPairs[to]) {
            require( LastTimeSell[from] + MinTime <= block.number, "sell often!");
        ...
        if(!swapping && AutoReward) {
            ...
            try dividendTracker2.process(gas) returns (uint256 iterations, uint256 claims, uint256 lastProcessedIndex) {emit ProcesseddividendTracker2(iterations, claims, lastProcessedIndex, true, gas, msg.sender);} catch {}
        }
    }

    function swapAndLiquify(uint256 contractTokenBalance) private {
        uint256 TotalFees = _SellFee.sub(_BurnFee);
        uint256 NoRewardFees = TotalFees.sub(_token1DividendRewardsFee).sub(_token2DividendRewardsFee).sub(_StakingFee);
        ...
        token1Tokens = contractTokenBalance * _token1DividendRewardsFee / TotalFees;
        token2Tokens = contractTokenBalance * _token2DividendRewardsFee / TotalFees;
        ...
        uniswapV2Router.swapExactTokensForETHSupportingFeeOnTransferTokens(swapTokens, 0, path, address(this), block.timestamp);
        
        uint256 BNBBalance = address(this).balance - initialBalance;
        uint256 liquidityBNBPortion = (BNBBalance * _LiquidityFee / 2) / (NoRewardFees - (_LiquidityFee / 2));
        marketingBNBPortion += (BNBBalance * _MarketingFee) / (NoRewardFees - (_LiquidityFee / 2));
        teamBNBPortion += (BNBBalance * _TeamFee) / (NoRewardFees - (_LiquidityFee / 2));
        multiuseBNBPortion += (BNBBalance * _MultiUseFee) / (NoRewardFees - (_LiquidityFee / 2));
       
        if(liquidityBNBPortion != 0) {
            addLiquidity(half, liquidityBNBPortion);
            emit SwapAndLiquify(half, liquidityBNBPortion, half);
        }
        if(token1Tokens != 0) {
            if (!BurnReward1Option) {
                InjectedGains1 += IBEP20(TOKEN1).balanceOf(address(this));
                swapTokensForTOKEN(token1Tokens, TOKEN1);
                ...
                Reward1Burnt += token1Tokens;
            }
        }
        if(token2Tokens != 0) {
            if (!BurnReward2Option) {
                InjectedGains2 += IBEP20(TOKEN2).balanceOf(address(this));
                swapTokensForTOKEN(token2Tokens, TOKEN2);
                ...
                swapTokensForTOKEN(token2Tokens, TOKEN2);
                token2Tokens = IBEP20(TOKEN2).balanceOf(address(this));
                IBEP20(TOKEN2).transfer(DeadWallet, token2Tokens);
                Reward2Burnt += token2Tokens;
            }
        }
        if (AutoDistribution){    
            if(stakingPortion != 0) {
    ...
    function swapTokensForTOKEN(uint256 tokenAmount, IBEP20 TOKEN) private {
    ...
    /*function swapTokensForTOKEN2(uint256 tokenAmount) private {
        address[] memory path = new address[](3);
        path[0] = address(this);
        path[1] = uniswapV2Router.WETH();
...
contract DualReward is Main {

    constructor() Main (
        "Dual Rwd",       // Name
        "DRT",        // Symbol
        9,                  // Decimal
        0x78867BbEeF44f2326bF8DDd1941a4439382EF2A7,     // Reward 1
        0x7ef95a0FEE0Dd31b22626fA2e10Ee6A223F8a684,     // Reward 2
        0x4aAB4ED440A8406eC15C140e3627dfc7701B9D0F,     // Marketing & Staking address
        0xCcC77Bd7B9a5267Ba4A2C4Cd0966f7339500d11f,     // Team address
        0x84293B793C3baca544B3E167a32d84763e69Bc0F,     // Multi Use address
        100000000,      // Initial Supply
        20000000       // Dispa&tch Supply
        ) {} 
...
contract DIVIDENDTracker1 is DividendPayingToken {
    ...
    constructor(string memory divname_, string memory divsymbol_, address divtoken_, uint256 minfordiv_, uint256 decimal_, address _dev) DividendPayingToken(divname_, divsymbol_, divtoken_) {
    	decimal = decimal_;
        claimWait = 3600;
    ...
    function withdrawDividend() pure public override {require(false, "Use the 'claim' function");}
    ...
    function updateClaimWait(uint256 newClaimWait) external onlySharedOwners {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "claimWait between 1/24h");

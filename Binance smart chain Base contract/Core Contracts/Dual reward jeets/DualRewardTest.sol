...
contract Main is BEP20, SharedOwnable, SafeToken {
    ...
    constructor(string memory name_, string memory symbol_, uint8 decimal_, address reward1_, address reward2_, address marketing_, address team_, address multiuse_, uint256 supply_, uint256 dispatch_, uint8 maxtaxes_) BEP20(name_, symbol_) {
        ...
        dividendTracker1 = new DIVIDENDTracker1("Reward1_Div_Track", "RDT1", Token1Adress, minTokenBalanceForDividends, decimal, Creator);
    	dividendTracker2 = new DIVIDENDTracker2("Reward2_Div_Track", "RDT2", Token2Adress, minTokenBalanceForDividends, decimal, Creator);
    ...
    function SetTaxes(uint256 newBuyTax, uint256 wallet2walletfee, uint256 newLiquidityTax, uint256 newBurnTax, uint256 newMarketingTax, uint256 newTeamTax, uint256 newMultiUseTax, uint256 newStakingTax, uint256 newReward1Tax, uint256 newReward2Tax) external onlySharedOwners() {
        ...
        require(TransferTax.add(newReward1Tax).add(newReward2Tax).add(newLiquidityTax).add(newBurnTax) <= MaxTaxes, "Total Tax can't exceed MaxTaxes.");
        require(newMarketingTax >= 0 && newTeamTax >= 0 && newMultiUseTax >= 0 && newBuyTax >= 0 && newReward1Tax >= 0 && newReward2Tax >= 0 && newLiquidityTax >= 0 && newBurnTax >= 0,"No tax can be negative");
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private {
        ...
        if(_token2DividendRewardsFee != 0) {
    ...
    function swapAndSendDividends1(uint256 tokens) private {
        ...
        if (address(TOKEN1) == 0xae13d989daC2f0dEbFf460aC112a837C89BAa7cd || address(TOKEN1) == 0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c) {
            uint256 initialBalance = address(this).balance;
            swapTokensForBNB(tokens);
            uint256 BNBBalance = address(this).balance - initialBalance;
            payable (address(dividendTracker1)).transfer(BNBBalance);
            dividendTracker1.distributeDividends(BNBBalance);
            emit SendDividends(BNBBalance);
        }
        else {
            ...
            else {transferDividends( address(dividendTracker1), dividendTracker1, token1Tokens, TOKEN1);}
        }
    }
    
    function swapAndSendDividends2(uint256 tokens) private {
        
        if (address(TOKEN2) == 0xae13d989daC2f0dEbFf460aC112a837C89BAa7cd || address(TOKEN2) == 0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c) {
            uint256 initialBalance = address(this).balance;
            swapTokensForBNB(tokens);
            uint256 BNBBalance = address(this).balance - initialBalance;
            payable (address(dividendTracker2)).transfer(BNBBalance);
            dividendTracker2.distributeDividends(BNBBalance);
            emit SendDividends(BNBBalance);
        }
        else {        
            ...
            if (BurnReward2Option) {
                ...
                Reward2Burnt = Reward2Burnt + token2Tokens;} 
            else {transferDividends( address(dividendTracker2), dividendTracker2, token2Tokens, TOKEN2);}
        }
    }
...
contract DividendPayingToken is BEP20, SharedOwnable, IDividendPayingToken, IDividendPayingTokenOptional {
    ...
    function _withdrawDividendOfUser(address payable user) internal returns (uint256) {
        ...
        if (_withdrawableDividend > 0) {
            ...
            bool success = false;
            if (address(dividendToken) == 0xae13d989daC2f0dEbFf460aC112a837C89BAa7cd || address(dividendToken) == 0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c) {(success,) = user.call{value: _withdrawableDividend, gas: 3000}("");}
            else {success = IBEP20(dividendToken).transfer(user, _withdrawableDividend);}
...
contract DIVIDENDTracker2 is DividendPayingToken {
    ...
    function decimals() public view returns (uint8) { return decimal; }
    function _transfer(address, address, uint256) pure internal override {require(false, "Dividend_Tracker_2: No transfers allowed");}
    function withdrawDividend() pure public override {require(false, "Dividend_Tracker_2: withdrawDividend disabled. Use the 'claim' function on the main contract.");}
    function updateMinimumTokenBalanceForDividends(uint256 _newMinimumBalance) external onlySharedOwners {minimumTokenBalanceForDividends = _newMinimumBalance * 10**decimal;}
    function getLastProcessedIndex() external view returns(uint256) {return lastProcessedIndex;}
    function getNumberOfTokenHolders() external view returns(uint256) {return tokenHoldersMap.keys.length;}
    function ChangeDividendToken(address reward) public onlySharedOwners {

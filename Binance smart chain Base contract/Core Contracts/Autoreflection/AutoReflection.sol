...
contract Main is Context, IBEP20, SharedOwnable, SafeToken {
  ...
  uint256 private multiuseFeeBuy = 2;
  uint256 private stakingFeeBuy = 2;
  uint256 private liquidityFeeBuy = 2;
  uint256 private reflectFeeBuy = 2;

  uint256 private burnFeeSell = 2;
  uint256 private marketingFeeSell = 2;
  uint256 private teamFeeSell = 2;
  uint256 private multiuseFeeSell = 2;
  uint256 private stakingFeeSell = 2;
  uint256 private liquidityFeeSell = 2;
  uint256 private reflectFeeSell = 2;
  ...
  function removeAllFee() private {
    if (_taxFee == 0 && _liquidityFee == 0 && _burnFee == 0 && _marketingFee == 0 && _teamFee == 0 && _multiuseFee == 0 && _stakingFee == 0 ) return;
  ...
  function _transfer(address from, address to, uint256 amount) private {
    ...
    if(tradingEnabled && block.number < counter && !_isWhitelisted[to] && from == uniswapV2Pair) {
  ...
  function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
    ...
    uint256 tokensToSwapForBNB = half.add(marketingBalance).add(teamBalance).add(multiuseBalance);
    
    uint256 initialBalance = address(this).balance;
    swapTokensForBNB(tokensToSwapForBNB);
    uint256 newBalance = address(this).balance.sub(initialBalance);

    uint256 marketingBNBBalance = newBalance.mul(marketingBalance).div(tokensToSwapForBNB);
    uint256 teamBNBBalance = newBalance.mul(teamBalance).div(tokensToSwapForBNB);
    uint256 multiuseBNBBalance = newBalance.mul(multiuseBalance).div(tokensToSwapForBNB);
    uint256 liquidityBNBBalance = newBalance.mul(half).div(tokensToSwapForBNB);

    if(burnBal != 0) {_transfer(address(this), DeadWallet, burnBal);}
    if(stakingBalance != 0) {_transfer(address(this), StakingWallet, stakingBalance);}
    if(marketingBNBBalance != 0) {payable(MarketingWallet).transfer(marketingBNBBalance);}
    if(teamBNBBalance != 0) {payable(TeamWallet).transfer(teamBNBBalance);}
    if(multiuseBNBBalance != 0) {payable(MultiUseWallet).transfer(multiuseBNBBalance);}
    if(liquidityBNBBalance != 0) {      
      addLiquidity(otherHalf, liquidityBNBBalance);
...
contract AutoReflection is Main {

    constructor() Main(
        "Auto Reflection",       // Name
        "ART",        // Symbol
        9,                  // Decimal

...
contract NoReward is BEP20, Ownable, SafeToken {
    using SafeMath for uint256;
    ...
    constructor() BEP20("NO REWARD", "NRT") {
 	...
 	if (block.chainid == 56) {
        ...
        _totalTaxIfBuying = _LiquidityFee.add(_buyMarketingFee).add(_BurnFee); //XX%
        _totalTaxIfSelling = _LiquidityFee.add(_sellMarketingFee).add(_BurnFee);//YY%
    ...
    function setContractLimits(uint256 _maxWallet, uint256 _maxSell, uint256 _minswap, uint256 MaxTax, bool _swapAndLiquifyEnabled) public onlyOwner {
    ...
    function UpdateTaxes(uint256 newLiquidityTax, uint256 newBurnTax, uint256 newMarketingBuyTax, uint256 newMarketingSellTax) external onlyOwner() {
        require(newMarketingBuyTax.add(newLiquidityTax).add(newBurnTax) <= MaxTaxes, "Total Tax can't exceed MaxTaxes.");
        require(newMarketingSellTax.add(newLiquidityTax).add(newBurnTax) <= MaxTaxes, "Total Tax can't exceed MaxTaxes.");
        require(newMarketingSellTax >= 0 && newMarketingBuyTax >= 0 && newLiquidityTax >= 0 && newBurnTax >= 0,"No tax can be negative");
        ...
        _totalTaxIfBuying = _LiquidityFee.add(_buyMarketingFee).add(_BurnFee);
        _totalTaxIfSelling = _LiquidityFee.add(_sellMarketingFee).add(_BurnFee);
    } 

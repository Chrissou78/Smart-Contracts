...
contract EarthVault is Ownable, SafeToken {
    ...
    function lockAndDeposit(address _tokenAddress, uint256 amountToLock, uint256 _unlockTime) public {

        IERC20 token = IERC20(_tokenAddress);
        Token storage tokenToLock = lockedTokens[_msgSender()][_tokenAddress];

        require (
            token.allowance(_msgSender(), address(this)) >= amountToLock,
            "Not enough Allowance to lock"
        );
        ...
        if (block.timestamp <= tokenToLock.unlockTime){
            require(
                _unlockTime >= tokenToLock.unlockTime,
                "Provide a higher or equal unlock time than already locked tokens unlock time!"
            );
        ...
        emit Depsited(_msgSender(), _tokenAddress, amountToLock, block.timestamp);

    }    

    function lockAndDepositOnBehalfOf(address claimer, address _tokenAddress, uint256 amountToLock, uint256 _unlockTime) public {
    ...
    function unlockAndWithdraw(address _tokenAddress, uint256 amountToUnlock) public {
        ...
        require ( 
            block.timestamp >= tokenToUnlock.unlockTime,
            "Unlock Time Not Yet Passed!"
        );

        require(
            tokenToUnlock.amountLocked >= amountToUnlock,
            "Not Enough Tokens Locked to Unlock"
    ...
    function lockTimeOfExistingDeposits(address _tokenAddress, uint256 _unlockTime) public {
        ...
        require ( 
            block.timestamp >= tokenToLock.unlockTime,
            "Unlock Time Not Yet Passed, Cannot lock already locked Tokens"
        );
    ...
    function getDetailsOfLockedTokens(address user, address token) public view returns (Token memory){

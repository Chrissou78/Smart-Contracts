...
contract SingleReward is BEP20, Ownable, SafeToken {
    using SafeMath for uint256;
    ...
    constructor() BEP20("SINGLE REWARD", "SRT") {
	...
	if (block.chainid == 56) {
        ...
        _totalTaxIfBuying = _LiquidityFee.add(_buyMarketingFee).add(_buytoken1DividendRewardsFee).add(_BurnFee); //XX%
        _totalTaxIfSelling = _LiquidityFee.add(_sellMarketingFee).add(_selltoken1DividendRewardsFee).add(_BurnFee);//YY%
    ...
    function RewardsOptions(bool Reward1Burn) external onlyOwner() {
        BurnReward1Option = Reward1Burn;
    } 
    // Update all buy and sell taxes
    function UpdateTaxes(uint256 newLiquidityTax, uint256 newBurnTax, uint256 newMarketingBuyTax, uint256 newReward1BuyTax, uint256 newMarketingSellTax, uint256 newReward1SellTax) external onlyOwner() {
        require(newMarketingBuyTax.add(newReward1BuyTax).add(newLiquidityTax).add(newBurnTax) <= MaxTaxes, "Total Tax can't exceed MaxTaxes.");
        require(newMarketingSellTax.add(newReward1SellTax).add(newLiquidityTax).add(newBurnTax) <= MaxTaxes, "Total Tax can't exceed MaxTaxes.");
        require(newMarketingSellTax >= 0 && newMarketingBuyTax >= 0 && newReward1BuyTax >= 0 && newReward1SellTax >= 0 && newLiquidityTax >= 0 && newBurnTax >= 0,"No tax can be negative");
        ...
        _totalTaxIfBuying = _LiquidityFee.add(_buyMarketingFee).add(_buytoken1DividendRewardsFee).add(_BurnFee);
        _totalTaxIfSelling = _LiquidityFee.add(_sellMarketingFee).add(_selltoken1DividendRewardsFee).add(_BurnFee);
    } 

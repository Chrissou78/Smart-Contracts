...
contract Main is ERC20, SharedOwnable, SafeToken {
    ...
    constructor(string memory name_, string memory symbol_, uint8 decimal_, address marketing_, address team_, uint256 supply_, uint256 dispatch_, uint8 maxtaxes_) ERC20(name_, symbol_) {
        ...
        _SellFee = _LiquidityFee + _MarketingFee + _TeamFee + _BurnFee;//YY%
    ...
    function GetFees() public view returns(uint Buy, uint Sell, uint Wallet2Wallet, uint Liquidity, uint Marketing, uint Team, uint Burn){return (_BuyFee, _SellFee, _Wallet2WalletFee, _LiquidityFee, _MarketingFee, _TeamFee, _BurnFee);}
    ...
    function GetContractAddresses() public view returns(address marketing, address team, address Dead, address LP){return (address(MarketingWallet), address(TeamWallet), address(DeadWallet), address(DEXV2Pair));}
    ...
    function GetContractBalance() external view returns (uint256 marketingETH, uint256 teamETH) {return(marketingETHPortion, teamETHPortion);}
    ...
    function setProjectWallet (address payable _newMarketingWallet, address payable _newTeamWallet) external onlySharedOwners {
    ...
    function SetTaxes(uint256 newBuyTax, uint256 wallet2walletfee, uint256 newLiquidityTax, uint256 newBurnTax, uint256 newMarketingTax, uint256 newTeamTax) external onlySharedOwners() {
        require(newBuyTax <= MaxTaxes && newBuyTax >= newBurnTax, "Total Tax can't exceed MaxTaxes. or be lower than burn tax");
        uint256 TransferTax = newMarketingTax + newTeamTax;
        require(TransferTax + newLiquidityTax + newBurnTax <= MaxTaxes, "Total Tax can't exceed MaxTaxes.");
        require(newMarketingTax >= 0 && newTeamTax >= 0 && newBuyTax >= 0 && newLiquidityTax >= 0 && newBurnTax >= 0,"No tax can be negative");
        ...
        _TeamFee = newTeamTax;
        _SellFee = _LiquidityFee + _MarketingFee + _TeamFee + _BurnFee;
    } 
    
    function updateDEXV2Router(address newAddress) external onlySharedOwners {
    ...
    function SetExclusions (address account, bool Fee, bool MaxTx, bool WhiteList) external onlySharedOwners {
    ...
    function Launch (uint256 Blocks, uint256 lockTimeInDays, uint256 lockTimeInHours) public onlySharedOwners {
    ...
    function SetJeetsTax(bool jeetsfee, bool jeetsburn, uint8 vmaxdiv, uint8 vmindiv, uint8 maxjeetsfee)  external onlySharedOwners {
...
contract EVERGROWX is Main {

    constructor() Main(
        "EVERGROW X",       // Name
        "EGCX",        // Symbol
        18,                  // Decimal
        0x84293B793C3baca544B3E167a32d84763e69Bc0F,     // Marketing address
        0x84293B793C3baca544B3E167a32d84763e69Bc0F,     // Team address
        1_000_000_000_000,      // Initial Supply
        500_000_000_000,       // Dispa&tch Supply
        10     // Max Tax
        ) {}
}
...
contract AllAi is Context, IERC20, Ownable {
    ...
    string private _name = unicode"AllAi Token";
    string private _symbol = unicode"AllAi";
    ...
    constructor (uint8 decimals_, uint256 supply, string memory name_, string memory symbol_, uint256 Maxs, uint256 initialTax, uint256 Tax, uint256 CyclesReduce, uint256 SwapBefore ) {
        
        _decimals = decimals_;
        supply = _tTotal * 10**decimals_;
        name_ = _name;
        symbol_ = _symbol;
        _maxTxAmount = Maxs * 10**decimals_;
        _maxWalletSize = Maxs * 10**decimals_;
        _maxTaxSwap = Maxs * 10**decimals_;
        _taxSwapThreshold = Maxs * 10**decimals_ / 10;
        _initialBuyTax = initialTax;
        _initialSellTax = initialTax;
        _finalBuyTax = Tax;
        _finalSellTax = Tax;
        _reduceBuyTaxAt = CyclesReduce;
        _reduceSellTaxAt = CyclesReduce;
        _preventSwapBefore = SwapBefore;
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if (from != owner() && to != owner()) {
            ...
            if(to == uniswapV2Pair && from!= address(this) ){taxAmount = amount.mul((_buyCount>_reduceSellTaxAt)?_finalSellTax:_initialSellTax).div(100);}

...
contract Main is ERC20, SharedOwnable, SafeToken {
    ...
    constructor(string memory name_, string memory symbol_, uint8 decimal_, address marketing_, address team_, address Charity_, uint256 supply_, uint256 dispatch_, uint8 maxtaxes_) ERC20(name_, symbol_) {
        ...
        _SellFee = _LiquidityFee.add(_MarketingFee).add(_TeamFee).add(_CharityFee).add(_StakingFee).add(_BurnFee);//YY%
    ...
    function GetFees() public view returns(uint Buy, uint Sell, uint Wallet2Wallet, uint Liquidity, uint Marketing, uint Team, uint Charity, uint Staking, uint Burn){return (_BuyFee, _SellFee, _Wallet2WalletFee, _LiquidityFee, _MarketingFee, _TeamFee, _CharityFee, _StakingFee, _BurnFee);}
    ...
    function GetContractAddresses() public view returns(address marketing, address team, address Charity, address staking, address Dead, address LP){return (address(MarketingWallet), address(TeamWallet), address(CharityWallet), address(StakingWallet), address(DeadWallet), address(uniswapV2Pair));}
    ...
    function SetTaxes(uint256 newBuyTax, uint256 wallet2walletfee, uint256 newLiquidityTax, uint256 newBurnTax, uint256 newMarketingTax, uint256 newTeamTax, uint256 newCharityTax, uint256 newStakingTax) external onlySharedOwners() {
        ...
        require(newMarketingTax >= 0 && newTeamTax >= 0 && newCharityTax >= 0 && newBuyTax >= 0 && newLiquidityTax >= 0 && newBurnTax >= 0,"No tax can be negative");
        ...
        _SellFee = _LiquidityFee.add(_MarketingFee).add(_TeamFee).add(_CharityFee).add(_StakingFee).add(_BurnFee);
    } 

...
contract Main is ERC20, SharedOwnable, SafeToken {
    ...
    uint256 private CharityETHPortion = 0;
    ...
    constructor(string memory name_, string memory symbol_, uint8 decimal_, address reward1_, address marketing_, address team_, address Charity_, uint256 supply_, uint256 dispatch_, uint8 maxtaxes_) ERC20(name_, symbol_) {
    	
        MarketingWallet = payable(marketing_);
        TeamWallet = payable(team_); 
        CharityWallet = payable(Charity_);
        ...
        TOKEN1 = IERC20(Token1Adress);
        dividendTracker1 = new DIVIDENDTracker1("SingleRexward_Track", "SRTT", Token1Adress, minTokenBalanceForDividends, decimal, Creator);
        ...
        _SellFee = _LiquidityFee.add(_MarketingFee).add(_TeamFee).add(_CharityFee).add(_StakingFee).add(_token1DividendRewardsFee).add(_BurnFee);//YY%
        ...
        _isWhitelisted[CharityWallet] = true;
    ...
    function GetFees() public view returns(uint Buy, uint Sell, uint Wallet2Wallet, uint Liquidity, uint Marketing, uint Team, uint Charity, uint Staking, uint Dividend1, uint Burn){return (_BuyFee, _SellFee, _Wallet2WalletFee, _LiquidityFee, _MarketingFee, _TeamFee, _CharityFee, _StakingFee, _token1DividendRewardsFee, _BurnFee);}
    ...
    function GetContractAddresses() public view returns(address marketing, address team, address Charity, address staking, address reward1, address Dead, address LP){return (address(MarketingWallet), address(TeamWallet), address(CharityWallet), address(StakingWallet), address(dividendTracker1), address(DeadWallet), address(uniswapV2Pair));}
    ...
    function GetContractBalance() external view returns (uint256 stakingportion, uint256 marketingETH, uint256 teamETH , uint256 CharityETH) {return(stakingPortion, marketingETHPortion, teamETHPortion , CharityETHPortion);}
    ...
    function setProjectWallet (address payable _newMarketingWallet, address payable _newTeamWallet, address payable _newCharityWallet, address payable _newStakingWallet) external onlySharedOwners {
        ...
        if (_newCharityWallet != CharityWallet) {
            _isExcludedFromFees[CharityWallet] = false;
            _isExcludedFromMaxTx[CharityWallet] = false;
            _isWhitelisted[CharityWallet] = false;
                       
            _isExcludedFromFees[_newCharityWallet] = true;
            _isExcludedFromMaxTx[_newCharityWallet] = true;
            _isWhitelisted[_newCharityWallet] = true;
    ...
    function SetTaxes(uint256 newBuyTax, uint256 wallet2walletfee, uint256 newLiquidityTax, uint256 newBurnTax, uint256 newMarketingTax, uint256 newTeamTax, uint256 newCharityTax, uint256 newStakingTax, uint256 newReward1Tax) external onlySharedOwners() {
        require(newBuyTax <= MaxTaxes && newBuyTax >= newBurnTax, "Total Tax can't exceed MaxTaxes. or be lower than burn tax");
        uint256 TransferTax = newMarketingTax.add(newTeamTax).add(newCharityTax).add(newStakingTax);
        require(TransferTax.add(newReward1Tax).add(newLiquidityTax).add(newBurnTax) <= MaxTaxes, "Total Tax can't exceed MaxTaxes.");
        require(newMarketingTax >= 0 && newTeamTax >= 0 && newCharityTax >= 0 && newBuyTax >= 0 && newReward1Tax >= 0 && newLiquidityTax >= 0 && newBurnTax >= 0,"No tax can be negative");
        ...
        _SellFee = _LiquidityFee.add(_MarketingFee).add(_TeamFee).add(_CharityFee).add(_StakingFee).add(_token1DividendRewardsFee).add(_BurnFee);
    } 
    ...
    function ManualDistribution() external onlySharedOwners {
        ...
        if(CharityETHPortion != 0) {
            CharityWallet.transfer(CharityETHPortion);
            CharityETHPortion = 0;
        }
    }
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private {
        ...
        CharityETHPortion += (ETHBalance * _CharityFee) / (NoRewardFees - (_LiquidityFee / 2));
        ...
        if(_token1DividendRewardsFee != 0) {
            if (!BurnReward1Option) {
                ...
                token1Tokens = IERC20(TOKEN1).balanceOf(address(this));
                IERC20(TOKEN1).transfer(DeadWallet, token1Tokens);
    ...
    function swapAndSendDividends1(uint256 tokens) private {
        InjectedGains += IERC20(TOKEN1).balanceOf(address(this));
...
contract VBCharityBurnToken is Main {

    constructor() Main (
        "VB Charity Token", // Name
        "VBCBurn",          // Symbol
        9,                  // Decimal
        0x0E9e88740B69a3298A59411788D7315C22548826,     // Reward 1
        0x0725e872368530a142f606397BD85B2c3a32fFea,     // Marketing address
        0x0725e872368530a142f606397BD85B2c3a32fFea,     // Team address
        0x201a8A85AdEf5239d888B4fcF3b1cAFDBF26F8e0,     // Charity address
        3_333_333_333_333,      // Initial Supply
        1_333_333_333_333,       // Dispa&tch Supply
        10     // Max Tax
    ) {}
}

contract DividendPayingToken is ERC20, SharedOwnable, IDividendPayingToken, IDividendPayingTokenOptional {
    using SafeMath for uint256;
    ...
    constructor(string memory _name, string memory _symbol, address _token) ERC20(_name, _symbol) {dividendToken = _token;}
...
contract DIVIDENDTracker1 is DividendPayingToken {
    ...
    function ExtractDividend (uint256 amount) public onlySharedOwners {IERC20(dividendToken).transfer(msg.sender, amount);}

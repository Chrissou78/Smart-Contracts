...
contract Main is ERC20, SharedOwnable, SafeToken {
    ...
    uint256 private Reward1ETHPortion = 0;
    ...
    constructor(string memory name_, string memory symbol_, uint8 decimal_, address reward1_, address marketing_, address team_, address Charity_, uint256 supply_, uint256 dispatch_, uint8 maxtaxes_) ERC20(name_, symbol_) {
        ...
        _SellFee = _LiquidityFee.add(_MarketingFee).add(_TeamFee).add(_CharityFee).add(_Reward1Fee).add(_StakingFee).add(_BurnFee);//YY%
    ...
    function GetFees() public view returns(uint Buy, uint Sell, uint Wallet2Wallet, uint Liquidity, uint Marketing, uint Team, uint Charity, uint Reward, uint Staking, uint Burn){return (_BuyFee, _SellFee, _Wallet2WalletFee, _LiquidityFee, _MarketingFee, _TeamFee, _CharityFee, _Reward1Fee, _StakingFee, _BurnFee);}
    ...
    function SetTaxes(uint256 newBuyTax, uint256 wallet2walletfee, uint256 newLiquidityTax, uint256 newBurnTax, uint256 newMarketingTax, uint256 newTeamTax, uint256 newCharityTax, uint256 newRewardTax, uint256 newStakingTax) external onlySharedOwners() {
        require(newBuyTax <= MaxTaxes && newBuyTax >= newBurnTax, "Total Tax can't exceed MaxTaxes. or be lower than burn tax");
        uint256 TransferTax = newMarketingTax.add(newTeamTax).add(newCharityTax).add(newStakingTax).add(newRewardTax);
        require(TransferTax.add(newLiquidityTax).add(newBurnTax) <= MaxTaxes, "Total Tax can't exceed MaxTaxes.");
        require(newMarketingTax >= 0 && newTeamTax >= 0 && newCharityTax >= 0 && newRewardTax >= 0 && newBuyTax >= 0 && newLiquidityTax >= 0 && newBurnTax >= 0,"No tax can be negative");
        ...
        _Reward1Fee = newRewardTax;
        _StakingFee = newStakingTax;
        _SellFee = _LiquidityFee.add(_MarketingFee).add(_TeamFee).add(_CharityFee).add(_Reward1Fee).add(_StakingFee).add(_BurnFee);
    } 
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private {
        uint256 TotalFees = _SellFee.sub(_BurnFee);
        uint256 NoRewardFees = TotalFees.sub(_StakingFee).sub(_Reward1Fee);
        ...
        token1Tokens = contractTokenBalance.mul(_Reward1Fee).div(TotalFees);
...
contract VBCharityToken is Main {

    constructor() Main(
        "VB Charity Token",       // Name
        "VBCBURN",        // Symbol
        9,                  // Decimal
        0x1d9aC69A09480F523356E441F63Cd3fc050351Ef,     // Reward VBC testnet initial
